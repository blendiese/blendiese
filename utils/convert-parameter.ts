...
export const convertParameter = (parameter: Parameter) => {
  const timeNow = new Date();
  let pointInTime: Date;
  ...
  if (parameter.timeSelect.mode === "relative") {
    switch (parameter.timeSelect.since) {
      ...
      case "custom":
      default:
        pointInTime = parameter.timeSelect.customTime ??
          new Date(timeNow.getTime() - 1000 * 60 * 60 * 24 * 7);
        break;
    }

    return pointInTime.toISOString();
  }
